...
const HomePage = () => {
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            const newExpense = values.type === 'expense' ? values.amount : 0;
            const newIncome = values.type === 'income' ? values.amount : 0;

            
            if (totalExpense + newExpense > totalIncome + newIncome) {
                setLoading(false);
                return message.error("Expense exceeds total income!");
            }
